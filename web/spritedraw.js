...
class ViewLayoutTool extends Tool {
    handle_touch_events(type, event, touchPos, gx, gy, deltaX, deltaY, field, toolBar) {
        if (this.event_handler)
            this.event_handler(type, event, touchPos, gx, gy, deltaX, deltaY, field, toolBar);
    }
    constructor(layoutManager, name, path, event_handler) {
        super(name, path);
...
class OutlineTool extends ExtendedTool {
    handle_touch_events(type, event, touchPos, gx, gy, deltaX, deltaY, field, toolBar) {
        const startTouchPos = [field.zoom.invZoomX(toolBar.drawingScreenListener.startTouchPos[0]), field.zoom.invZoomY(toolBar.drawingScreenListener.startTouchPos[1])];
        ...
        switch (type) {
            case ("touchstart"):
                field.layer().autoOutline(new Pair(gx, gy), toolBar.outLineTool.checkboxOnlyOneColor.checked);
                break;
            case ("touchmove"):
    ...
    handle_touch_events(type, event, touchPos, gx, gy, deltaX, deltaY, field, toolBar) {
        switch (type) {
            ...
            case ("touchmove"):
                ...
                const startTouchPos = [(toolBar.field.zoom.invZoomX(toolBar.drawingScreenListener.startTouchPos[0]) / field.width()) * field.width(),
...
class PenViewTool extends ViewLayoutTool {
    constructor(pen, name, path, event_handler) {
...
class PenTool extends ExtendedTool {
    ...
    constructor(strokeWith, toolName = "pen", pathToImage = ["images/penSprite.png"], optionPanes, field, dimLocal = [200, 160]) {
        ...
        this.tbSize = new GuiTextBox(true, 90, null, 16, 35, GuiTextBox.default, (event) => {
...
class SprayCanTool extends PenTool {
    constructor(strokeWidth, toolName, pathToImage, callBack, optionPanes, field) {
        ...
        this.tbProbability = new GuiSlider(1, [125, 40], (event) => {
...
class ToolSelector {
    constructor(pallette, keyboardHandler, drawingScreenListener, imgWidth = 64, imgHeight = 64) {
        ...
        this.touchListener.registerCallBack("hover", (event) => true, (event) => {
        ...
        this.touchListener.registerCallBack("doubletap", (event) => true, (event) => {
            this.transformEvent(event);
            event.translateEvent(event, this.tool().getOptionPanel().x, this.tool().getOptionPanel().y);
            this.tool().getOptionPanel().handleTouchEvents("doubletap", event);
            this.repaint = true;
        });
        this.touchListener.registerCallBack("longtap", (event) => true, (event) => {
            this.transformEvent(event);
            event.translateEvent(event, this.tool().getOptionPanel().x, this.tool().getOptionPanel().y);
            this.tool().getOptionPanel().handleTouchEvents("longtap", event);
            this.repaint = true;
        });
        this.touchListener.registerCallBack("tap", (event) => true, (event) => {
            this.transformEvent(event);
            event.translateEvent(event, this.tool().getOptionPanel().x, this.tool().getOptionPanel().y);
        ...
        this.toolBar.tools.push(new PenViewTool(this.penTool, "line", ["images/ThePixelSlime1Icons/LineDrawSprite.png", "images/LineDrawSprite.png"], (type, event, touchPos, gx, gy, deltaX, deltaY, field) => {
            const x1 = touchPos[0] - deltaX;
            const y1 = touchPos[1] - deltaY;
            switch (type) {
                case ("touchend"):
        ...
        this.toolBar.tools.push(new PenViewTool(this.penTool, "rect", ["images/ThePixelSlime1Icons/rectSprite.png", "images/rectSprite.png"], (type, event, touchPos, gx, gy, deltaX, deltaY, field) => {
            switch (type) {
                case ("touchend"):
                    if (this.penTool.checkboxPixelPerfect.checked)
        ...
        this.toolBar.tools.push(new PenViewTool(this.penTool, "oval", ["images/ThePixelSlime1Icons/ovalSprite.png", "images/ovalSprite.png"], (type, event, touchPos, gx, gy, deltaX, deltaY, field) => {
            switch (type) {
                case ("touchend"):
        ...
        this.toolBar.tools.push(new ViewLayoutTool(this.copyTool.getOptionPanel(), "paste", ["images/ThePixelSlime1Icons/pasteSprite.png", "images/pasteSprite.png"], (type, event, touchPos, gx, gy, deltaX, deltaY, field) => {
            switch (type) {
            }
        }));
        this.toolBar.tools.push(this.dragTool);
        this.toolBar.tools.push(new ViewLayoutTool(this.undoTool.localLayout, "redo", ["images/ThePixelSlime1Icons/redoSprite.png", "images/redoSprite.png"], (type, event, touchPos, gx, gy, deltaX, deltaY, field) => {
            switch (type) {
            }
        }));
...
class DrawingScreen {
    ...
    getSelectedPixelGroupBitMask(startCoordinate, countColor) {
    ...
    handleDraw(x1, x2, y1, y2, drawPoint = (x, y, screen) => screen.handleTap(x, y)) {
        ...
        const b = y2 - m * x2;
        const delta = this.state.lineWidth <= 4 ? 0.9 : (this.state.drawCircular ? (this.state.lineWidth < 16 ? 2 : this.state.lineWidth / 16) : 1);
...
async function main() {
    ...
    const ctx = maybectx;
    let field;
    const multiTouchHandler = new MultiTouchListener(canvas, false, true, false);
    multiTouchHandler.registerCallBack("pinch", (e) => {
        {
            let delta = 0;
